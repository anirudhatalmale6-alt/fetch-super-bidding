...
class LoginController extends ApiController
{
    ...
    {
        // Get roles allowed for web login - exclude mobile and dispatch roles, but include owner
        $web_login_roles = RoleSlug::whereNotIn('slug', array_merge(Role::dispatchRoles()))
            ->pluck('slug')
            ->toArray(); 
    ...
    protected function authenticateUser(User $user, $remember = false)
    {
        ...
        // Check if user has owner role - redirect to company dashboard
        ...
        return $this->respondSuccess()->header('Cache-Control', 'no-cache, no-store, must-revalidate')
                                        ->header('Pragma', 'no-cache')
                                        ->header('Expires', '0');
    }
