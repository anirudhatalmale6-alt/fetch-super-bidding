...
class OwnerController extends BaseController
{
    ...
    public function store(StoreOwnerRequest $request){
        ...
        // Add company type if specified
        if($request->has('company_type')){
            $created_params['company_type'] = $request->input('company_type');
        }
        ...
        // Create Trucking Company if type is trucking or both
        if(in_array($request->input('company_type'), ['trucking', 'both'])){
            $truckingCompany = TruckingCompany::create([
                'company_name' => $request->input('company_name'),
                'slug' => \Str::slug($request->input('company_name') . '-' . uniqid()),
                'registration_number' => $request->input('email'), // Use email as temp registration
                'email' => $request->input('email'),
                'phone' => $request->input('mobile'),
                'user_id' => $user->id,
                'company_type' => $request->input('company_type') === 'both' ? 'both' : 'interstate_trucking',
                'status' => 'pending',
                'fleet_size' => $request->input('no_of_vehicles', 0),
            ]);

            // Link owner to trucking company
            $owner->update([
    ...
    public function update(Owner $owner,UpdateOwnerRequest $request)
    {
        ...
        $validate_exists_mobile = $this->owner->where('id','!=',$owner->id)->where('mobile',$request->mobile)->exists();
